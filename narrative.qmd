...
Now, when you edit this file, it will show up as changed in the Git panel. Replace the `YOUR NAME HERE` part with your actual name and then save the file. Now, render the file to see the differences as represented in HTML format.

![](media/rendering.png)

Notice in the Git Pane it now lists that file as changed.

![](media/hw1.png)

This file is not *staged* (e.g., it is not set up to be committed yet). If we select the file in the pane and hit the *Diff* button just above it, a Diff window shows us the actual changes that were made to that file.

![The red highlight shows that line 3 was removed and the green one shows that the 3rd line was replaced with the new content. Everything else in the file is left as it was.](media/diff.png)
